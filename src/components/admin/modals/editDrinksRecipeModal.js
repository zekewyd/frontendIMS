...
function EditDrinksRecipeModal({ recipe, onClose, onSubmit }) {
    // Form state
    ...
    useEffect(() => {
        if (recipe) {
            ...
            setSelectedProduct(recipe.ProductID?.toString() || "");
            ...
            if (recipe.Materials && recipe.Materials.length > 0) {
                const formattedSupplies = recipe.Materials.map(mat => ({
                    supplyId: mat.MaterialID.toString(),
                    quantity: mat.Quantity.toString(),
                    size: mat.Measurement
                }));
    ...
    const handleSubmit = async (e) => {
        ...
        if (Object.keys(newErrors).length === 0) {
            ...
            try {
                const response = await fetch(`${RECIPE_API_URL}/recipes/${recipe.RecipeID}`, {
                    method: "PUT",
                    headers: {
            ...
            <div className="add-recipe-modal-container">
                ...
                <form className="add-recipe-modal-form" onSubmit={handleSubmit}>
                    ...
                    <div className="add-recipe-button-container">
                        <button className="add-recipe-submit-button" type="submit">Update Recipe</button>
                    </div>
                </form>
            </div>
        </div>
...
export default EditDrinksRecipeModal;